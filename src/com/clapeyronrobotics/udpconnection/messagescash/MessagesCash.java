...
public class MessagesCash {
                ...
                for (int i = 0; i < size1; i++) {
                    ...
                    for (int j = 0; j < addressCash.getWaitingMessagesSize(); j++) {
                        ...
                        if (currentTime - timerTimeout >= message.getTimestamp()) {
                            ...
                            MessagesCash.this.messageDeliveryListener.onDeliveryTimeout(addressCash.getInetAddress(), addressCash.getPort(), message);
                            ...
                            if (currentTime - timerPeriod >= message.getTimestamp()) {
                                ...
                                MessagesCash.this.messageDeliveryListener.onDeliveryRepeatSending(addressCash.getInetAddress(), addressCash.getPort(), message);
                            }
                        }
                    }
                }
    ...
    public void removeMessageByAnswer(InetAddress inetAddress, int port, Message message) {
        if (getAddressCash(inetAddress,port).removeMessageByAnswer(message))
            messageDeliveryListener.onDelivered(inetAddress, port, message);
    }
